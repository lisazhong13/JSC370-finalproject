...
downloadable report link: https://github.com/lisazhong13/JSC370-finalproject/JSC370-Project.pdf
```{r include=FALSE}
...
To enhance the analysis and address my research questions more comprehensively, I supplemented the DHDS dataset with additional socioeconomic and demographic variables. Specifically, I scraped uninsured rate data from the American Community Survey (ACS) using the U.S. Census Bureau API. Furthermore, I downloaded various datasets from the Kaiser Family Foundation (KFF) website, including information on the poverty rate by age, healthcare access indicators such as health insurance coverage, hospital counts, Medicaid spending, and additional disability-related data. For this analysis, poverty is defined as the proportion of individuals living below the federal poverty line within each state. This is calculated as the percentage of individuals whose income falls below a specified threshold set by the federal government, adjusted annually for inflation. This metric provides insight into the economic conditions in each state and is used as a key indicator for analyzing socioeconomic disparities and their relationship with disability prevalence. Since Medicare is available primarily to individuals aged 65+, the variables `MedicareInsuranceCoverage` and `TotalMedicareEnrollees` serve as proxies for age structure across states.
...
#### Summary Statistics
A summary of the top 10 states with the highest disability prevalence is shown below (Table 1), including medians and interquartile ranges for key variables:
...
summary_top5 <- df_final %>%
  filter(State %in% top_states) %>%
  select(DisabilityPrevalence, DisabilityPopulation, UninsuredRate, PovertyTotal, 
...
# Display
kable(summary_top5, caption = "Table 1: Median [IQR] Summary Statistics for Top 5 States by Disability Prevalence in 2021")
```
...
knitr::kable(anova_summary_table, caption = "Table 2: ANOVA Results for Disability Proportions Across States")
```
