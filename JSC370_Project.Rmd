...
summary_top3_linebreak <- summary_top3 %>%
  mutate(across(-State, ~ gsub(" \\[", " \\\\ [", .x))) %>%  # insert actual LaTeX linebreak
  mutate(across(-State, ~ paste0("\\makecell[l]{", .x, "}")))  # [l] = left-align

# Transpose
transposed_table <- summary_top3_linebreak %>%
